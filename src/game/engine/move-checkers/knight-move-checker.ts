...
export class KnightMoveChecker extends MoveChecker implements MoveValidator {
  ...
  getAllKnightMovesFromPosition(piece: Piece, pos: Coordinate, grid: Grid) {
    ...
    for (let xDirection = -1; xDirection <= 1; xDirection += 2) {
      for (let yDirection = -1; yDirection <= 1; yDirection += 2) {
        const movePos = { x: pos.x + moveWidth * xDirection, y: pos.y + moveLength * yDirection }
        const invertedMovePos = { x: pos.x + moveLength * xDirection, y: pos.y + moveWidth * yDirection }
  ...
  computeKnightMove(piece: Piece, oldPos: Coordinate, newPos: Coordinate, grid: Grid): Move | undefined {
    if(
